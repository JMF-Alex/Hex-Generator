...
const savedColors = ['C03AFE', '5B03E4', getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor()];
const presets = {
  1: {
...
function darkMode() {
  if (document.getElementById('darkmode').checked == true) {
    ...
    Array.from(document.getElementsByClassName("hexColor")).forEach(e => {
      document.getElementById(e.id).classList.add("darkcolorboxes");
    })
  } else {
...
function updateOutputText(event) {
  ...
  if (format.outputPrefix) {
    ...
    if (nickName.value) {
      ...
      if (!nickName.value.match(letters)) nickName.value = 'Tect.host';
    }
  }
